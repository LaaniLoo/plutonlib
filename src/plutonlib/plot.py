...
def plot_extras(sdata,pdata = None, **kwargs):
    ...
    if sdata.sim_type in ("Jet"):
        title = f"{sdata.sim_type} {labels[1]}/{labels[0]} Across {coord_labels[0]}/{coord_labels[1]} ({sdata.run}, {pdata.d_file})"
        title_other.append(title)

    #assigning title if other: one var per subplot
    if sdata.sim_type in ("Stellar_Wind"):
        title_L = f"{sdata.sim_type} {labels[0]} Across {coord_labels[0]}/{coord_labels[1]} ({sdata.run}, {pdata.d_file})"
        title_R = f"{sdata.sim_type} {labels[1]} Across {coord_labels[0]}/{coord_labels[1]} ({sdata.run}, {pdata.d_file})"
...
def plot_save(sdata,pdata=None,custom=0,file_type = "png", **kwargs):
    """
    ...
    * if custom = 1: used to label the file by the function it was called rather than sim_type/run
    ...
    save = input(f"Save plot for {sdata.run}? [1/0]: ")
    if save == "1":
        if custom:
...
def plot_sim(sdata,sel_d_files = None,sel_runs = None,sel_prof = None, pdata = None,**kwargs):
    ...
    * multiple plots can be generated by specifying sel_runs, also will override sdata.run
    """
    if pdata is None:
        pdata = PlotData(**kwargs)
    ...
    for run in run_names:
        ...
        sdata = pl.SimulationData(sdata.sim_type,sdata.run,sdata.profile_choice,auto_load = False)
    ...
    for d_file in pdata.d_files: # plot across all files
        ...
        for coord in sel_coords: #TODO possibly use zip? 
            for var_name in sel_vars:
                sel_var = pdata.vars[var_name]
                sel_coord = pdata.vars[coord]
                ...
                coord_units = sdata.get_var_info(coord)["si"]
                var_label = sdata.get_var_info(var_name)["var_name"]
                var_units = (sdata.get_var_info(var_name)["si"]).to_string('latex')
                ...
                ax.set_title(
                    f"{title_str} vs {coord_label} ({sdata.run}, {d_file})"
                )
