...
def pcmesh_3d_nc(sdata,pdata = None, **kwargs):    
    ...
    vars_data = np.log10(sdata.get_vars()[var_name][profile]) if is_log else sdata.get_vars()[var_name][profile]
...
def cmap_base(sdata,pdata = None, **kwargs):
    ...
    * Note needs to be looped over d_files and var_name for all info to be loaded (see plot_sim)
    ...
    if pu.sim_type_match(sdata)["is_stellar_wind"]:
        raise NotImplementedError("check Stellar Wind working with pcmesh_3d_nc")
...
def plotter(sel_coord,sel_var,sdata,sel_d_files = None,**kwargs):
    ...
    for d_file in pdata.d_files: # plot across all files
        ...
        var_array = sdata.get_vars(d_file)[sel_var]
        coord_array = sdata.get_vars(d_file)[sel_coord]
